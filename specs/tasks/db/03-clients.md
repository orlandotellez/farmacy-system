...
### 1. Tabla de Clientes
- [x] Crear tabla `client` (full_name, document_type, document_number, phone, email, address, birth_date, sex).
- [x] Datos de salud: `allergies`, `chronic_diseases`, `observations`.
- [x] `is_frequent`, `store_id`, `deleted_at`.
- [x] Índices: `full_name`, `document_number`, `phone`, `[store_id, full_name]`.
...
- [ ] Índice sobre `[store_id, is_frequent]` si se filtra por frecuentes con frecuencia (ya existe `idx_client_store_is_frequent`).

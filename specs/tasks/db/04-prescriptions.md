...
### 1. Recetas
- [x] Crear tabla `prescription` (number, doctor, centro, fechas, imagen, notas, status, cliente, store).
- [x] Unique `(store_id, number)`.
- [x] Índices: `status`, `client_id`, `[store_id, created_at]`, `[store_id, status]`.
- [x] FK `client_id` con `ON DELETE SET NULL` (receta anónima permitida).

### 2. Items de Receta
- [x] Crear tabla `prescription_item` (medicine_name snapshot, quantity).
- [x] Autorización: `authorized_quantity` (default 0) y `authorized_by`.
- [x] Cascade delete desde `prescription`.

### 3. Pendientes / Mejoras
- [x] Índice `[store_id, expiry_date]` sobre prescriptions (para listar recetas vencidas automáticamente).
